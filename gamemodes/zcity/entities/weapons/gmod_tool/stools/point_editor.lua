...
function TOOL:Think()
	...
	if ply:KeyDown(IN_ATTACK2) then
		if !self.LastHold then
			self.LastHold = CurTime()
		end

		if !self.Trace1 then
			self.Trace1 = util.QuickTrace(ply:EyePos(), ply:EyeAngles():Forward() * 999999, ply)
		end
		...
		if SERVER and self.IsHolding then
			...
			for k, v in pairs(zb.Points) do
				...
				for i = #v.Points, 1, -1 do
					if self.Trace1.HitPos:Distance(v.Points[i].pos) <= distance then
						zb.RemoveMapPoint( k, i, true)
					end
				end
			end
...
function TOOL.BuildCPanel( CPanel )
	...
	dlist.OnRowSelected = function( lst, index, pnl )
		RunConsoleCommand("point_editor_point", pnl:GetValue(1))
	end
end
...
function TOOL:DrawHUD()
	...
	for id, points in pairs(zb.ClPoints) do
		for id2, point in pairs(points) do
			...
			local txtsize, txtsizey = surface.GetTextSize(text)
